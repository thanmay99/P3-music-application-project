...
function Home() {
  ...
  const deletePlaylist = (playlistName) => {
    ...
    localStorage.setItem('allPlaylist', JSON.stringify(updatedPlaylists));
  };
      ...
      <div className="container">
        ...
        <div className="row">
          {Object.keys(playlists).map((playlistName) => (
            <div key={playlistName} className="col-lg-3 col-md-6 py-2 d-flex align-items-stretch">
              <div className="card bg-dark text-white shadow-lg d-flex flex-column">
                <div className="card-body d-flex flex-column">
                  <h5 className="card-title">{playlistName}</h5>
                  <Link to={`/playlist/${playlistName}`} className="btn btn-primary mt-auto">
                    View Songs
                  </Link>
                  <button
